...
#
# Before sending to the model, ``collate_batch`` function works on a batch of samples generated from ``DataLoader``. The input to ``collate_batch`` is a batch of data with the batch size in ``DataLoader``, and ``collate_batch`` processes them according to the data processing pipelines declared previouly. Pay attention here and make sure that ``collate_batch`` is declared as a top level def. This ensures that the function is available in each worker.
#
